...
!
MODULE aed_phyto_particles
!-------------------------------------------------------------------------------
! aed_phyto_particles --- test phytoplankton particle model
!
...
!-------------------------------------------------------------------------------
   ...
   PRIVATE
!
   PUBLIC aed_phyto_particles_data_t
!
   TYPE,extends(aed_model_data_t) :: aed_phyto_particles_data_t
...
!ARGUMENTS
   ...
   CLASS (aed_phyto_particles_data_t),INTENT(inout) :: data
!
!LOCALS
   INTEGER  :: status

!  %% NAMELIST    %%  /aed_phyto_particles/
...
!                                             ! 3 = other metrics
!                                             !10 = all debug & checking outputs
!  %% END NAMELIST    %%  /aed_phyto_particles/

   NAMELIST /aed_phyto_particles/ vvel_new, vvel_old, &
...
!BEGIN
   ...
   ! Read the namelist
   read(namlst,nml=aed_phyto_particles,iostat=status)
   IF (status /= 0) STOP 'Error reading namelist aed_phyto_particles'

   print *,"        aed_phyto_particles initialization"
...
!ARGUMENTS
   CLASS (aed_phyto_particles_data_t),INTENT(in) :: data
...
END MODULE aed_phyto_particles
